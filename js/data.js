...
(function () {
  ...
  var generateComments = function (number, textData, nameData) {
    ...
    for (var i = 0; i < number; i++) {
      var message = textData[window.util.getRandom(0, textData.length - 1)];
      if (window.util.getRandom(0, 1)) {
        message += ' ' + textData[window.util.getRandom(0, textData.length - 1)];
      }
      result.push({
        avatar: 'img/avatar-' + window.util.getRandom(1, 6) + '.svg',
        message: message,
        name: nameData[window.util.getRandom(0, nameData.length - 1)]
      });
    }
    return result;
  };
  ...
  var generateData = function (number) {
    ...
    for (var i = 1; i <= number; i++) {
      result.push({
        ...
        comments: generateComments(window.util.getRandom(1, 10), USER_COMMENTS, USER_NAME)
      });
    }
    return result;
  };
